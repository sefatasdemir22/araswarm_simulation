...
# 1. MAĞARA DÜNYASI
gnome-terminal --tab --title="Cave World" -- bash -c "cd ~/araswarm_ws; source /opt/ros/humble/setup.bash; source ~/araswarm_ws/install/setup.bash; ros2 launch gas_distribution_sim cave_simulation.launch.py; exec bash"
...
# 3. DRONE SPAWN
gnome-terminal --tab --title="Spawn Drone" -- bash -c "source ~/src/PX4-Autopilot/Tools/simulation/gazebo-classic/setup_gazebo.bash ~/src/PX4-Autopilot ~/src/PX4-Autopilot/build/px4_sitl_default; source /opt/ros/humble/setup.bash; source ~/araswarm_ws/install/setup.bash; ros2 run gazebo_ros spawn_entity.py -entity iris -file ~/src/PX4-Autopilot/Tools/simulation/gazebo-classic/sitl_gazebo-classic/models/iris/iris.sdf -x -2.0 -y 0.0 -z 0.2; exec bash"
